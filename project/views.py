...
def enterprise_detail(request, enterprise_id):
    ...
    # Проверка на равенство количества сотрудников и количества заполненных анкет
    all_employees_surveyed = (enterprise.employees == SurveyResponse.objects.filter(enterprise=enterprise).count())
    
    context = {
        'enterprise': enterprise,
        'has_records': has_records,
        'survey_url': survey_url,
        'all_employees_surveyed': all_employees_surveyed,
    }
...
def survey(request, enterprise_id):
    ...
    if all_employees_surveyed:
        return HttpResponse("Анкетирование завершено для всех сотрудников.")
